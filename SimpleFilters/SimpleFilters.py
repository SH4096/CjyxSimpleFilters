...
import os,sys
import unittest
from __main__ import vtk, qt, ctk, cjyx
...
For detailed information about a specific filter please consult the <a href=\"http://www.itk.org/Doxygen/html/\">Insight Toolkit Doxygen</a>.
 """.format(parent.cjyxWikiUrl, cjyx.app.majorVersion, cjyx.app.minorVersion)

    parent.acknowledgementText = """
The developers would like to thank the support of the Cjyx Community, the Insight Toolkit and the ITK Community."
...
class SimpleFiltersWidget:
  def __init__(self, parent = None):
    ...
    if not parent:
      ...
      self.parent.setDRMLScene(cjyx.drmlScene)
    else:
      self.parent = parent
  ...
  def onApplyButton(self):
    try:
      ...
      qt.QMessageBox.critical(cjyx.util.mainWindow(),
...
class SimpleFiltersLogic:
  ...
  def cmdStartEvent(self, sitkFilter):
    #print "cmStartEvent"
    widget = cjyx.modules.SimpleFiltersWidget
  ...
  def thread_doit(self,sitkFilter,*inputImages):
    try:
      ...
      self.yieldPythonGIL()
      self.main_queue.put(lambda :qt.QMessageBox.critical(cjyx.util.mainWindow(),
  ...
  def main_queue_start(self):
    ...
    cjyx.modules.SimpleFiltersWidget.onLogicRunStart()
  ...
  def main_queue_stop(self):
    ...
    if self.thread.is_alive():
      self.thread.join()
    cjyx.modules.SimpleFiltersWidget.onLogicRunStop()
  ...
  def updateOutput(self,img):
    ...
    nodeWriteAddress=sitkUtils.GetCjyxITKReadWriteAddress(self.outputNodeName)
    sitk.WriteImage(img,nodeWriteAddress)

    node = cjyx.util.getNode(self.outputNodeName)
  ...
  def run(self, filter, outputDRMLNode, outputLabelMap, *inputs):
    """
    ...
    for i in inputs:
      ...
      img = sitk.ReadImage(sitkUtils.GetCjyxITKReadWriteAddress(imgNodeName) )
      inputImages.append(img)

    self.output = None
    # check
    self.outputNodeName = outputDRMLNode.GetName()
...
class FilterParameters:
  ...
  def create(self, json):
    ...
    if json["template_code_filename"] == "KernelImageFilter":
      ...
      fiducialSelector = cjyx.qDRMLNodeComboBox()
      self.widgets.append(fiducialSelector)
      fiducialSelector.nodeTypes = ( "vtkDRMLMarkupsFiducialNode", "vtkDRMLAnnotationHierarchyNode")
      fiducialSelector.addAttribute("vtkDRMLAnnotationHierarchyNode", "MainChildType", "vtkDRMLAnnotationFiducialNode" )
      ...
      fiducialSelector.setDRMLScene( cjyx.drmlScene )
      fiducialSelector.setToolTip( "Pick the Markups node for the seed list." )

      fiducialSelector.connect("nodeActivated(vtkDRMLNode*)", lambda node,name=name:self.onFiducialListNode(name,node))
      self.widgetConnections.append((fiducialSelector, "nodeActivated(vtkDRMLNode*)"))
    ...
    for member in json["members"]:
      ...
      if "dim_vec" in member and int(member["dim_vec"]):
        if member["itk_type"].endswith("IndexType") or member["itk_type"].endswith("PointType"):
          ...
          fiducialSelector = cjyx.qDRMLNodeComboBox()
          self.widgets.append(fiducialSelector)
          fiducialSelector.nodeTypes = (  "vtkDRMLMarkupsFiducialNode", "vtkDRMLAnnotationFiducialNode" )
          fiducialSelector.selectNodeUponCreation = True
          ...
          fiducialSelector.connect("nodeActivated(vtkDRMLNode*)", lambda node,w=fiducialSelector,name=member["name"],isPt=isPoint:self.onFiducialNode(name,w,isPt))
          self.widgetConnections.append((fiducialSelector, "nodeActivated(vtkDRMLNode*)"))
        ...
        fiducialSelector.connect("nodeActivated(vtkDRMLNode*)", lambda node,name=member["name"]:self.onFiducialListNode(name,node))
        self.widgetConnections.append((fiducialSelector, "nodeActivated(vtkDRMLNode*)"))
    ...
    self.outputSelector = cjyx.qDRMLNodeComboBox()
    self.widgets.append(self.outputSelector)
    self.outputSelector.nodeTypes = ["vtkDRMLScalarVolumeNode", "vtkDRMLLabelMapVolumeNode"]
    ...
    self.outputSelector.setDRMLScene( cjyx.drmlScene )
    self.outputSelector.setToolTip( "Pick the output to the algorithm." )

    self.outputSelector.connect("nodeActivated(vtkDRMLNode*)", lambda node:self.onOutputSelect(node))
    self.widgetConnections.append((self.outputSelector, "nodeActivated(vtkDRMLNode*)"))
  ...
  def createInputWidget(self,n, noneEnabled=False):
      inputSelector = cjyx.qDRMLNodeComboBox()
      self.widgets.append(inputSelector)
      inputSelector.nodeTypes = ["vtkDRMLScalarVolumeNode", "vtkDRMLLabelMapVolumeNode"]
      ...
      inputSelector.setDRMLScene( cjyx.drmlScene )
      inputSelector.setToolTip( "Pick the input to the algorithm." )

      # connect and verify parameters
      inputSelector.connect("nodeActivated(vtkDRMLNode*)", lambda node,i=n:self.onInputSelect(node,i))
      self.widgetConnections.append((inputSelector, "nodeActivated(vtkDRMLNode*)"))
  ...
  def onInputSelect(self, drmlNode, n):
    self.inputs[n] = drmlNode

  def onOutputSelect(self, drmlNode):
    self.output = drmlNode
    self.onOutputLabelMapChanged(drmlNode.IsA("vtkDRMLLabelMapVolumeNode"))

  def onOutputLabelMapChanged(self, v):
    self.outputLabelMap = v
    self.outputLabelMapBox.setChecked(v)

  def onFiducialNode(self, name, drmlWidget, isPoint):
    if not drmlWidget.visible:
      return
    annotationFiducialNode = drmlWidget.currentNode()

    # point in physical space
    coord = [0,0,0]

    if annotationFiducialNode.GetClassName() == "vtkDRMLMarkupsFiducialNode":
      # cjyx4 Markups node
  ...
  def onFiducialListNode(self, name, drmlNode):
    annotationHierarchyNode = drmlNode

    # list of points in physical space
    coords = []

    if annotationHierarchyNode.GetClassName() == "vtkDRMLMarkupsFiducialNode":
      # cjyx4 Markups node
